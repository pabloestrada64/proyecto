...
if (isset($_POST['form1'])) {
    ...
    if(empty($_POST['cust_name'])) {
        $valid = 0;
        $error_message .= "Nombre no puede estar vacio. ";
    }

    if(empty($_POST['cust_phone'])) {
        $valid = 0;
        $error_message .= "Numero de Telefono no puede estar vacio. ";
    }

    if(empty($_POST['cust_address'])) {
        $valid = 0;
        $error_message .= "Direccion no puede estar vacio. ";
    }

    if(empty($_POST['cust_country'])) {
        $valid = 0;
        $error_message .= "Pais no puede estar sin seleccionar. ";
    }

    if(empty($_POST['cust_city'])) {
        $valid = 0;
        $error_message .= "Ciudad no puede estar vacio. ";
    }

    if(empty($_POST['cust_state'])) {
        $valid = 0;
        $error_message .= "Estado no puede estar vacio";
    }

    if(empty($_POST['cust_zip'])) {
        $valid = 0;
        $error_message .= "Codigo Zip no puede estar vacio. ";
    }
                ...
                <div class="user-content">
                    ...
                    <form action="" method="post">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 form-group">
                                <label for=""><?php echo 'Ciudad' ?> *</label>
                            ...
                            <div class="col-md-6 form-group">
                                <label for=""><?php echo 'Codigo Zip' ?> *</label>
                                <input type="text" class="form-control" name="cust_zip" value="<?php echo $_SESSION['customer']['cust_zip']; ?>">
                            </div>
                        </div>
                        <input type="submit" class="btn btn-primary" value="Actualizar" name="form1">
                    </form>
                </div>                
            </div>
        </div>
